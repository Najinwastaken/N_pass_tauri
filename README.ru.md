...
2. Нажмите иконку **копирования** (⧉) — она первая из кнопок справа,
   сразу за точками пароля. На экране пароль не появится: он уходит прямо
   в буфер обмена. Кнопка отвечает зелёной галочкой, чтобы было видно,
...
В списке карт номер и CVV — отдельные колонки, у каждой своя кнопка
копирования и свой глаз: чтобы продиктовать CVV, незачем показывать на
экране весь номер карты.
