...
- **Категории** для паролей. Вписали категорию в запись — она есть;
  ...
  оставляет на экране только одну из них. Заголовок можно взять за ручку
  и расставить категории в нужном порядке, переименовать категорию
  целиком через меню `...` на заголовке — все её записи меняются за один
  раз, — а кнопка рядом с выпадающим списком сворачивает и разворачивает
  сразу все. Какие группы вы оставили свёрнутыми, приложение помнит между
  запусками, а порядок хранится внутри самого хранилища и потому
  переезжает вместе с его копией.
